...
(function($) {
	...
	$.fn.doAutosize = function(o) {
		...
		var testerWidth = testSubject.width(),
			newWidth = (testerWidth + o.comfortZone) >= minWidth ? testerWidth + o.comfortZone :
			minWidth,
	...
	$.fn.resetAutosize = function(options) {
		...
		var minWidth = $(this).data('minwidth') || options.minInputWidth || $(this)
			.width(),
			maxWidth = $(this).data('maxwidth') || options.maxInputWidth || ($(this).closest(
				'.tagsinput').width() - options.inputPadding),
			val = '',
			input = $(this),
	...
	$.fn.addTag = function(value, options) {
		...
		this.each(function() {
			...
			if (value != '' && skipTag != true) {
				...
				if (options.callback && tags_callbacks[id] && tags_callbacks[id][
						'onAddTag'
					]) {
	...
	$.fn.tagsInput = function(options) {
		...
		this.each(function() {
			...
			var markup = '<div id="' + id +
				'_tagsinput" class="tagsinput"><div id="' + id + '_addTag">';
			...
			if (settings.interactive) {
				...
				if (settings.autocomplete_url != undefined) {
					...
					if (jQuery.Autocompleter !== undefined) {
						...
						$(data.fake_input).bind('autocompleteselect', data, function(event,
							ui) {
					...
					$(data.fake_input).bind('blur', data, function(event) {
						...
						if ($(event.data.fake_input).val() != '' && $(event.data.fake_input)
							.val() != d) {
							if ((event.data.minChars <= $(event.data.fake_input).val().length) &&
								(!event.data.maxChars || (event.data.maxChars >= $(event.data.fake_input)
									.val().length)))
				...
				data.removeWithBackspace && $(data.fake_input).bind('keydown', function(
					event) {
